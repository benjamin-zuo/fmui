...
(function( fmui, $ ) {
    ...
    /**
     * Event类，结合fmui.event一起使用, 可以使任何对象具有事件行为。包含基本`preventDefault()`, `stopPropagation()`方法。
     ...
     * var obj = {};
     *
     * $.extend( obj, fmui.event );
     *
     * var etv = fmui.Event( 'beforeshow' );
     * obj.trigger( etv );
     *
     ...
     * }
     * ```
     * @grammar new fmui.Event( name[, props]) => instance
    ...
    fmui.event = {
         ...
         * @param  {String | Event }   evt     事件名或fmui.Event对象实例
         * @param  {*} * 任意参数
         * @return {self} 返回自身，方便链式
         * @chainable
         */
        trigger: function( evt ) {
            ...
            if ( events ) {
                ...
                while ( ++i < len ) {
                    if ( (stoped = evt.isPropagationStopped()) 
                        || false === (ev = events[ i ]).cb.apply( ev.ctx2, args )
                            ) {
