...
JpegMeta.parseSnum = function parseSnum(endian, data, offset, size) {
    ...
    for (big_endian ? i = offset : i = offset + size - 1; big_endian ? i < offset + size : i >= offset; big_endian ? i++ : i--) {
        if (neg === undefined)
            neg = (data.charCodeAt(i) & 128) === 128;
        ret <<= 8;
        ret += neg ? ~data.charCodeAt(i) & 255 : data.charCodeAt(i)
    }
    if (neg) {
        ret += 1;
        ret *= -1
...
JpegMeta.JpegFile = function JpegFile(binary_data, filename) {
    ...
    while (pos < this._binary_data.length) {
        ...
        if (mark_fn)
            this[mark_fn](mark, pos_start_of_segment +
                2)
    }
...
JpegMeta.JpegFile.prototype._SOI_MARKER = "\u00ff\u00d8";
JpegMeta.JpegFile.prototype._DELIM = 255;
JpegMeta.JpegFile.prototype._EOI = 217;
JpegMeta.JpegFile.prototype._SOS = 218;
JpegMeta.JpegFile.prototype._sofHandler = function _sofHandler(mark,
    pos) {
    if (this.general !== undefined)
...
JpegMeta.JpegFile.prototype._JFIF_IDENT =
    "JFIF\x00";
...
JpegMeta.JpegFile.prototype._gpstags = {
    ...
    30: ["GPS differential correction",
        "GPSDifferential"
    ]
};
...
JpegMeta.JpegFile.prototype._addMetaGroup = function _addMetaGroup(name, description) {
    ...
    this.metaGroups[group.fieldName] =
        group;
    return group
};
JpegMeta.JpegFile.prototype._parseIfd = function _parseIfd(endian, _binary_data, base, ifd_offset, tags, name, description) {
    ...
    for (var i = 0; i < num_fields; i++) {
        ...
        tag_field = JpegMeta.parseNum(endian, _binary_data,
            tag_base, 2);
        ...
        if (type == "UNDEFINED")
            ...
            for (j = 0; j < num_values; j++, value_offset += type_size) {
                ...
                if (type == "RATIONAL") {
                    ...
                    den = JpegMeta.parseNum(endian,
                        _binary_data, value_offset + 4, 4);
...
JpegMeta.JpegFile.prototype._jfifHandler = function _jfifHandler(mark, pos) {
    ...
    this.jfif._addProperty("Xdensity", "X density", JpegMeta.parseNum(">", this._binary_data, pos +
        8, 2));
...
JpegMeta.JpegFile.prototype._exifHandler = function _exifHandler(mark, pos) {
    ...
    if (this.tiff.ExifIfdPointer)
        this._parseIfd(endian, this._binary_data, pos, this.tiff.ExifIfdPointer.value,
            this._exiftags, "exif", "Exif");
